...
export default {
  data () {
    return {
      keywordList: wx.getStorageSync(KEYWORD_LIST_KEY) || []
    }
  },
  components: {
    SearchBar
  ...
  onHide () {
    // 跳转到搜索列表后，更新历史搜索列表
    this.keywordList = wx.getStorageSync(KEYWORD_LIST_KEY)
  },
  methods: {
    // 清空历史搜索
    clearAll () {
      ...
      wx.showModal({
        ...
        success: res => {
          if (res.confirm) {
            ...
            this.keywordList = []
            //  存储清空
            wx.removeStorageSync(KEYWORD_LIST_KEY)
          }
        }
      })
    },
    ...
    toSearchList (inputVal) {
      ...
      // 存到storage
      wx.setStorageSync(KEYWORD_LIST_KEY, _keywordList)

      wx.navigateTo({
        url: '/pages/search_list/main?catName=' + inputVal
      })
    }
  }
}
