...
7. https://gitee.com/mzliuyunlong/uni-yougou45

   1. 完成删除购物车功能
   2. 用了uniapp组件swiperAction

8. 购物车bug, 判断相等误写成赋值，eslint可以拯救这种错误
...
1. 购物车
   1. 从商品详情跳转
   2. 展示商品列表
      1. 数据来自两部分，接口+storage
      2. 在商品详情页面添加购物车
         1. 第一次添加，插入对象，num:1
         2. 后续添加, 直接改
      3. 根据storage购物车cart, goodsId来发请求
         1. 请求回来的数据不是有序goodsList
         2. 两个数据渲染，取值办不到。所以需要数据合并，
            1. 合并以cart为主
      4. 渲染页面
   3. 全选
      1. 商品的选中改全选的状态
...
2. 登录页
   1. 声明式获取用户信息
   2. wx.login()获取微信用户的唯一标识
   3. 调接口传以上两部分数据，拿到token，存起来
...
1. 提示用户允许权限是吗？
   1. 拒绝的时候提示
      ...
      1. 先用getSetting获取用户授权的状态，给程序员用的
      
         ![image-20200310094100704](assets/image-20200310094100704.png)
   2. 打开设置让用户允许
      
      1. openSetting打开授权界面，给用户用的
      
         ![image-20200310094117370](assets/image-20200310094117370.png)



#### 授权行为处理：

1. 用wx.getSetting来判断用户是否被拒绝
   1. **判断对应的权限是否是false**
2. 如果没有拒绝，就包含第一次发起或者已经授权
   1. 先发起授权，调接口
3. 如果被拒绝了，提示：用openSetting打开界面，让用户允许

#### 另一种方案（更优）

1. 直接发起授权
   1. 第一次发起或者已经授权，调接口拿到授权数据
   2. 拒绝的话，会进入fail, 提示打开设置，让用户允许权限
...
4. 微信里面的通讯地址，我->头像->我的地址，里面有微信的收货地址，这里同步微信的收货地址
5. 在手机里面，如果想清除小程序授权数据，直接删除小程序即可。
...
![image-20200310165028742](assets/image-20200310165028742.png)