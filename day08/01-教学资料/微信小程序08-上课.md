...
1. 视频声音小
2. 最后几天就要毕业了，无心学习，却还是要坚持到最后。黑马的度过了难忘的时光，还挺不舍。加油！加油！
   1. 学习是一辈子的事情
   2. 调整心态，继续学习。即使面试上了，工作中的学习也不能停止。
   3. 毕业后能有半年时间专注在学习上，是挺难得的。
...
1. 本地存储API
   ...
   3. 同步的API, 加Sync
      ...
      2. 或者fs.readFileSync
2. 加入购物车的功能
   1. 购物车，商品id，商品数量，和商品选中状态都存在storage里面。
   2. 购物车存储结构最好为{{}}，方便判断是否为第一次添加和取值
   3. 无论首次还是第N次添加，checked:true;第一次添加num:1,后续num++
3. 会完成购物车页面商品列表的渲染
   1. 购物车商品列表的渲染，是由storage里面购物车及接口数据两部分渲染
   2. 融合两部分数据渲染更方便，没有莫名的bug
4. 会完成购物车页面全选/反选的功能
   1. 计算属性的setter
   2. 数据的every方法
5. 会完成购物车页面修改商品数量的功能
   1. 两部分数据融合后，注册事件改商品数量。
